...
library(vegan)
### Here I am including the code Dr. Caz commented to include, but it hasn't so;lved the error messages. :/
Sys.setenv("ROOT_MICROBIOME_DIR"=".")
```

## Current Status: Stuck, code runs partially, then gives error messages, gets stuck on first element of loop.
...
source(paste(Sys.getenv('ROOT_MICROBIOME_DIR'), "/hdr.microbiome_methods.R"));
source(paste(Sys.getenv('ROOT_MICROBIOME_DIR'), "/hdr.base_methods.R"));
...

{
	...
	###############################################################################
	setwd(paste0(Sys.getenv('ROOT_MICROBIOME_DIR'), "/gwas/otus97", otuCutoff, "/", organ, "/phenotypes/")); ## 
